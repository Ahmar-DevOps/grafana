...
function updateDatasourcePluginResetOption(props, key) {
    var _a, _b;
    var config = props.options;
...
var alertSuccess = eventFactory('alert-success');
var alertWarning = eventFactory('alert-warning');
var alertError = eventFactory('alert-error');

var appEvents = /*#__PURE__*/Object.freeze({
...
/** Events */
var refresh = eventFactory('refresh');
var componentDidMount = eventFactory('component-did-mount');
var dataError = eventFactory('data-error');
var dataReceived = eventFactory('data-received');
var dataSnapshotLoad = eventFactory('data-snapshot-load');
var editModeInitialized = eventFactory('init-edit-mode');
var initPanelActions = eventFactory('init-panel-actions');
var panelChangeView = eventFactory('panel-change-view');
var panelInitialized = eventFactory('panel-initialized');
var panelSizeChanged = eventFactory('panel-size-changed');
var panelTeardown = eventFactory('panel-teardown');
var render = eventFactory('render');
var viewModeChanged = eventFactory('view-mode-changed');

var panelEvents = /*#__PURE__*/Object.freeze({
    refresh: refresh,
...
function getDisplayProcessor(options) {
    ...
    return function (value) {
        ...
        return { text: text, numeric: numeric, prefix: prefix, suffix: suffix };
    };
}
...
function prepareConfigValue(key, input, options) {
    if (numericFieldProps[key]) {
        var num = toNumber_1(input);
        if (isNaN(num)) {
            return null;
        }
        return num;
...
function setDynamicConfigValue(config, options) {
    var value = options.value;
    var v = prepareConfigValue(value.path, value.value, options);
    set_1(config, value.path, v);
}
/**
 * For numeric values, only valid numbers will be applied
 * for units, 'none' will be skipped
 */
function setFieldConfigDefaults(config, props) {
    var e_6, _a;
    if (props) {
        ...
        try {
            for (var keys_1 = __values(keys), keys_1_1 = keys_1.next(); !keys_1_1.done; keys_1_1 = keys_1.next()) {
                ...
                var val = prepareConfigValue(key, props[key]);
                if (val === null || val === undefined) {
                    continue;
                }
                set_1(config, key, val);
            }
        }
        catch (e_6_1) { e_6 = { error: e_6_1 }; }
        finally {
    ...
    if (options.data) {
        ...
        for (var s = 0; s < data.length && !hitLimit; s++) {
            ...
            for (var i = 0; i < series.fields.length && !hitLimit; i++) {
                ...
                scopedVars['__field'] = { text: 'Field', value: { name: name_1 } };
